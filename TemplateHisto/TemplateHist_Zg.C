#define TemplateHist_Zg_cxx
#include "TemplateHist_Zg.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "iostream"
...
void TemplateHist_Zg::Loop(TString name, TString fitVal, int iso)
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	std::stringstream ii;
	ii<<i+1;
	if(processCode==2 && pho1_parentage==1){
	  if(EB==1){
	    if(fitVal == "Sieie"){
...
int main(int argc, char* argv[]){
  ...
  for (int i = 0; i < argc; i++ ) {
    ...
    TemplateHist_Zg t("/uscmst1b_scratch/lpc1/old_scratch/lpceg/yurii/EnSc/sachiko/Zgg/NEW_WORK_09010/Zgg/RootForTemp/ZgSig/muon_Zg_"+fitVal+"_"+_iso[iso]+"_Sig.root");
    t.Loop("ROOT_Sig/muon_Sig_"+fitVal+"_"+_iso[iso]+".root", fitVal, iso);
  }
