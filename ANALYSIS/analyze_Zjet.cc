#define analyze_Zjet_cxx
#include "analyze_Zjet.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TTree.h"
...
double analyze_Zjet::dR(double eta1, double phi1, double eta2, double phi2) {
  double dphi = acos(cos(phi1 - phi2));
  double deta = eta1 - eta2;
  return sqrt(dphi*dphi + deta*deta);
}

bool analyze_Zjet::fidEtaPass(double Eta){
...
int analyze_Zjet::phoRegion(double absEta){
  int region = 0;
...
// tight muon ID
bool analyze_Zjet::muID2012(int i){
  bool pass = true;
...
float analyze_Zjet::MuIsolation(int i){
  float isolation;
...
void analyze_Zjet::clearVariables(){
  ele1_pt = 0;
  ele1_phi = 0;
  ele1_eta = 0;
  ele2_pt= 0;
...
void analyze_Zjet::Loop(TString name, TString fitVal, int iso)
{
  if (fChain == 0) return;
...
int main(int argc, char* argv[]){
  ...
  for (int i = 0; i < argc; i++ ) {
    ...
    analyze_Zjet t;
    t.Loop("muon_Data_Zjet_"+fitVal+"_"+Loose[iso]+".root", fitVal, iso);
  }
