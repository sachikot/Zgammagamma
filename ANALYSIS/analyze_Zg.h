...
// Wed May 14 15:50:41 2014 by ROOT version 5.32/00
// from TTree EventTree/Event data
// found on file: job_summer12_ttjets_1l_skimmed_eeg.root
//////////////////////////////////////////////////////////
...
class analyze_Zg {
 public :
  TTree          *fChain;   //!pointer to the analyze_Zgd TTree or TChain
  ...
  analyze_Zg(TTree *tree=0);
  virtual ~analyze_Zg();
  virtual Int_t    Cut(Long64_t entry);
  virtual Int_t    GetEntry(Long64_t entry);
  virtual Long64_t LoadTree(Long64_t entry);
  virtual void     Init(TTree *tree);
  virtual float    SF_mediumPhoID( float pt, float eta);
  virtual float    muIDSF( float pt, float eta);
  virtual float    IsoSF( float pt, float eta);
  virtual float    muTriggerSF( float eta1, float eta2);
  virtual float    PixSeedVeto( float pt, float eta);
  virtual void     clearVariables();
  virtual void     Loop(TString name, TString fitVal, TString TempSample, int iso);
...
#ifdef analyze_Zg_cxx
analyze_Zg::analyze_Zg(TTree *tree) : fChain(0) 
{
  // if parameter tree is not specified (or zero), connect the file
  // used to generate this class and read the Tree.
  TChain* chain = new TChain("EventTree");
  //  chain->Add("/uscmst1b_scratch/lpc1/old_scratch/lpceg/yurii/EnSc/sachiko/Zgg/NEW_SKIM/Muon/event10491843.root"); 
  chain->Add("/uscmst1b_scratch/lpc1/old_scratch/lpceg/yurii/EnSc/sachiko/Zgg/NEW_SKIM/Muon/CrossCheck/job_summer12_Zg_s10_skimmed_zgg.root");
  chain->Add("/uscmst1b_scratch/lpc1/old_scratch/lpceg/yurii/EnSc/sachiko/Zgg/NEW_SKIM/Muon/CrossCheck/job_summer12_DYJetsToLL_s10_skimmed_zgg.root");
  //  chain->Add("/uscmst1b_scratch/lpc1/old_scratch/lpceg/yurii/EnSc/sachiko/Zgg/NEW_SKIM/Muon/CrossCheck/job_summer12_Zg_skimmed_zgg.root");
  tree = chain;
  Init(tree);
}

analyze_Zg::~analyze_Zg()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t analyze_Zg::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t analyze_Zg::LoadTree(Long64_t entry)
{
...
void analyze_Zg::Init(TTree *tree)
{
...
void analyze_Zg::Show(Long64_t entry)
{
...
Int_t analyze_Zg::Cut(Long64_t entry)
{
